...
(def cfg {:store {:backend :sqlite
                  ...
                  ;; optional, but recommended if you want to control the
                  ;; store identity used by konserve/datahike
...
Konserve 0.9 requires a UUID under `:id`. If you omit it,
datahike-sqlite derives a stable UUID from the SQLite `:dbname` and
`:table`, but setting `:id` explicitly is recommended when you want that
identity to be under your control.
